...
| features.containerCollection.criSocketPath | Path to the container runtime socket (if different from Docker). |
| features.containerCollection.dockerSocketPath | Path to the docker runtime socket. |
| features.containerCollection.kubelet.agentCAPath | AgentCAPath is the container path where the kubelet CA certificate is stored. Default: '/var/run/host-kubelet-ca.crt' if hostCAPath is set, else '/var/run/secrets/kubernetes.io/serviceaccount/ca.crt' |
| features.containerCollection.kubelet.host.configMapKeyRef.key | The key to select. |
| features.containerCollection.kubelet.host.configMapKeyRef.name | Name of the referent. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names TODO: Add other useful fields. apiVersion, kind, uid? |
| features.containerCollection.kubelet.host.configMapKeyRef.optional | Specify whether the ConfigMap or its key must be defined |
| features.containerCollection.kubelet.host.fieldRef.apiVersion | Version of the schema the FieldPath is written in terms of, defaults to "v1". |
| features.containerCollection.kubelet.host.fieldRef.fieldPath | Path of the field to select in the specified API version. |
| features.containerCollection.kubelet.host.resourceFieldRef.containerName | Container name: required for volumes, optional for env vars |
| features.containerCollection.kubelet.host.resourceFieldRef.divisor | Specifies the output format of the exposed resources, defaults to "1" |
| features.containerCollection.kubelet.host.resourceFieldRef.resource | Required: resource to select |
| features.containerCollection.kubelet.host.secretKeyRef.key | The key of the secret to select from.  Must be a valid secret key. |
| features.containerCollection.kubelet.host.secretKeyRef.name | Name of the referent. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names TODO: Add other useful fields. apiVersion, kind, uid? |
| features.containerCollection.kubelet.host.secretKeyRef.optional | Specify whether the Secret or its key must be defined |
| features.containerCollection.kubelet.hostCAPath | HostCAPath is the host path where the kubelet CA certificate is stored. |
| features.containerCollection.kubelet.tlsVerify | TLSVerify toggles kubelet TLS verification. Default: true |
| features.containerCollection.liveContainerEnabled | Enables container collection for the Live Container View. Default: true |
| features.containerCollection.oomKillCheck | Enables the OOMKill eBPF-based check. Default: false |
| features.containerCollection.tcpQueueLengthCheck | Enables the TCP queue length eBPF-based check. Default: false |
...
| features.cws.enableSyscallMonitor | EnableSyscallMonitor enables Syscall Monitoring (recommended for troubleshooting only). Default: false |
...
| features.dogstatsd.originDetection | Enable origin detection for container tagging. See also: https://docs.datadoghq.com/developers/dogstatsd/unix_socket/#using-origin-detection-for-container-tagging |
...
| features.eventCollection.kubernetesEventsCollection |  |
...
| features.processCollection.enabled | Enabled enables Process monitoring. Default: false |

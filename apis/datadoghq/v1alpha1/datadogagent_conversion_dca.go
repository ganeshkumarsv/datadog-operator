...
func convertClusterAgentSpec(src *DatadogAgentSpecClusterAgentSpec, dst *v2alpha1.DatadogAgent) {
	...
	if src.Image != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Image = src.Image
	}

	if src.DeploymentName != "" {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Name = src.DeploymentName
	}
	...
	if src.Config != nil {
		...
		if src.Config.CollectEvents != nil {
			...
			setBooleanPtrOR(src.Config.CollectEvents, &features.EventCollection.KubernetesEventsCollection)
		}

		if src.Config.LogLevel != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).LogLevel = src.Config.LogLevel
		}

		if src.Config.Resources != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).Resources = src.Config.Resources
		}

		if src.Config.Command != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).Command = src.Config.Command
		}

		if src.Config.Args != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).Args = src.Config.Args
		}

		if src.Config.Confd != nil {
			getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).ExtraConfd = convertConfigDirSpec(src.Config.Confd)
		}

		if src.Config.Env != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).Env = src.Config.Env
		}

		if src.Config.VolumeMounts != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).VolumeMounts = src.Config.VolumeMounts
		}

		if src.Config.Volumes != nil {
			getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Volumes = src.Config.Volumes
		}

		if src.Config.HealthPort != nil {
			getV2Container(getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName), v2alpha1.ClusterAgentContainerName).HealthPort = src.Config.HealthPort
		}
	}

	if src.CustomConfig != nil {
		tmpl := getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName)
	...
	if src.Rbac != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).CreateRbac = src.Rbac.Create
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).ServiceAccountName = src.Rbac.ServiceAccountName
	}

	if src.Replicas != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Replicas = src.Replicas
	}

	if src.AdditionalAnnotations != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Annotations = src.AdditionalAnnotations
	}

	if src.AdditionalLabels != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Labels = src.AdditionalLabels
	}

	if src.PriorityClassName != "" {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).PriorityClassName = src.PriorityClassName
	}

	if src.Affinity != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Affinity = src.Affinity
	}

	if src.Tolerations != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).Tolerations = src.Tolerations
	}

	if src.NodeSelector != nil {
		getV2TemplateOverride(&dst.Spec, v2alpha1.ClusterAgentResourceName).NodeSelector = src.NodeSelector
	}
